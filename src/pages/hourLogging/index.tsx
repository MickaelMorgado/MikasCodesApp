...
export const HourLogging = ({ id }: IHourLoggingProps) => {
  const [checkpoints, setCheckpoints] = useState<ICheckpoint[]>(JSON.parse(getLocalStorageItem(Enum_StorageSlot.hourLogs)))
  ...
  const addCheckpoint = (checkpoint: ICheckpoint) => {
    setCheckpoints(prevState => {
      if (prevState.length !== 0) {
        const lastCheckpointModified = prevState[prevState.length - 1].date.end = checkpoint.date.start
      }
  ...
  return (
    <>
      <h3>Log Hours</h3>
      <PaddedContent>
        <Description><>WIP: Here you can log your hours</></Description>
      </PaddedContent>
      <Divider light />
      <PaddedContent>
        <S.Points>
          {checkpoints
            .map(
              (point, index) => {
                const previousPoint = checkpoints[index-1] ? checkpoints[index-1].date.start : point.date.start

                return RenderReactElementFromMap(
                  <S.PointLine>
                    <S.FirstCol>
                      <GS.React>
                        {formatDate(point.date.start, {format: Enum_FormatDate.exactTime})}
                      </GS.React> - {point.date.end ? <GS.Js>{formatDate(point.date.end, {format: Enum_FormatDate.exactTime})}</GS.Js> : <AppLoaderIcon />}
                    </S.FirstCol>
                    <GS.Terminal>
                      <TextField
                        fullWidth
